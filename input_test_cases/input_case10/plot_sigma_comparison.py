...
bipolar_lfp_rms = np.zeros(len(sigma))
bipolar_lfp_rms_DTI = np.zeros(len(sigma))
models = []
...
for j, type in enumerate(rms_values.keys()):
    for i in range(len(sigma)):
        ...
        fileending = f"5000_350_{type}_sigma{sigma[i]}_En0"
        ...
        if type == "myGillies":
            encap_values.append(f"{sigma[i]}") # in um
        # Calculate PSD
        ...
        if type == "myGillies":
            ...
            plt.plot(
                ...
                label=(f"sigma {sigma[i]}"),
            )
            #plt.title("LFP PSD (Welch's Method) " + type)
